...
public class Assorted {
    ...
    public static int findSum(List<?> list) {
        ...
        for(Object nums: list) {
            sum += Integer.parseInt(String.valueOf(nums));
        }
        return sum;
    }
    ...
    public static List<Integer> filterStrings(List list) {
        ...
        for(Object nums: list) {
            if(nums instanceof String){
            }else{
                balls.add((Integer) nums);
            }
        }
        return balls;
    }
    ...
    public static List<String> lineNumbering(List<String> list) {
        ...
        int ballss = 1;
        for(String bob: list) {
            balls.add(String.format("%1$h: %2$s", ballss, bob));
            ballss++;
        }
        return balls;
    }
    ...
    public static int toBinary(List<Integer> list) {
        ...
        for(int i = list.size()-1; i > -1; i --){
            num+= list.get(i) * Math.pow(2,imstupid);
            imstupid++;
        }
        return num;
    }
